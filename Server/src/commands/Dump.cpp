...
void Dump::execute(shared_ptr<ConnectionInfo> connection_info, vector<string> args) {
    
    if (args.size() < 1) {
        string msg = "You should pass <filename> argument for DUMP command.";
    ...
    string res = connection_info->databaseManager->dump(args[0]);
